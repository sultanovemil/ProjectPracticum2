...
В завершение работы была построена модель экстремального градиентного бустинга (XGBoost). В среде Colab полученные результаты RMSLE составили 0.39419.
...
## 5. Процесс предсказания
После завершения процесса формирования предсказания длительности поездок на тестовой выборке, был создан submission-файл и отправлен на платформу Kaggle для оценки значения метрики RMSLE на тестовой выборке. Полученные на платформе Kaggle результаты RMSLE составляют 0.40269 .
